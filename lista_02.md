...
B) ``RESPOSTA ESCOLHIDA`` O código avalia a expressão booleana, imprime o resultado `true`, calcula a soma dos números de 1 a 5 e imprime o resultado no console. 
...
B) ``RESPOSTA ESCOLHIDA``A saída de calcularOrcamento() seria: 'Seu saldo é negativo de -1050.' e a de calcularOrcamento2() seria: 'Seu saldo é negativo de -100.'
...
D) ``RESPOSTA ESCOLHIDA``O código verifica se o número é par, se é divisível por 3 ou se é ímpar. Se for par, exibe a mensagem "O número é par!". Se for divisível por 3 (e não for par), exibe a mensagem "O número é divisível por 3!". Se for ímpar (e não for divisível por 3), exibe a mensagem "O número é ímpar e não é divisível por 3!".
...
B) ``RESPOSTA ESCOLHIDA`` Preload -> Create -> Update
...
B) ``RESPOSTA ESCOLHIDA`` Simular interações físicas realistas, como colisões e movimentos, em jogos 2D.
...
``RESPOSTA``
```
ALGORITMO "Sistema de Voto"

VAR
	idade: inteiro

INICIO
	idade <- INSIRA("Digite sua idade: ")

	SE idade < 16 ENTAO
		ESCREVA ("Não pode votar!")
	SE NÃO
		SE idade < 18 E idade >=16 ENTÃO
...
Classe Retangulo HERDA FormaGeometrica:
    ...
    Método Construtor(cor, base, altura):
        - HERDAR (Define o valor do atributo cor da mesma forma que ocorre na classe herdada)
...
``RESPOSTA``
```
ALGORITMO "VerificaçãoTesla"

VAR
	tempoMaximo, velocidade, velocidadeInicial, aceleracao, tempo, velocidadeMaxima, tempoDeCorrida: inteiro

INICIO
    tempoMaximo <- 1800 //segundos vulgo meia hora
    velocidadeInicial <- 0
    aceleracao <- 20 //
    tempo <- 0
    velocidadeMaxima <- 120 //km/H
    velocidade = velocidadeInicial + aceleracao*tempo
    tempoDeCorrida <-0

    ENQUANTO tempoDeCorrida < tempoMaximo
        AGUARDE 1 segundo
        tempoDeCorrida <- tempoDeCorrida + 1
            ENQUANTO velocidade <= velocidadeMaxima
                AGUARDE 60 segundos
                tempo = tempo + 1
...
Função ProdutoDeMatrizes(matrizA, matrizB):
    ...
    SE NÃO::
        ...
        # Loop para calcular a multiplicação de matrizes
